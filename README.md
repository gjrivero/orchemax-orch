...
Every cell below is a fact from the 2026-09-15 orchestrator scan, extended by the 2026-09-22 source read, or *Not documented* — never a guess.
...
| Test quality | *Not documented* | *Not documented* | *Not documented* | *Not documented* | None — it is a terminal manager and runs no check of its own | **`orch verify mutate` breaks each branch the change adds, runs only the tests that touch it, and fails if one stays green; `orch guard test-dup` names a test whose body repeats another's** |
...
Same JSON shape Orchemax embeds for symbol seeding — the same files, in fact: a
test in the product fails when the two copies disagree. A seed also works without
this repo, at `.orch/langs/<id>.json` in your own workshop
([how](https://docs.orchemax.com/how-to/add-your-language/)).
